...
The project employs GitHub Actions for Continuous Integration and Continuous Deployment, with separate pipelines for the frontend and backend. Both pipelines are triggered on pushes and pull requests to the \texttt{main} branch, scoped to their respective directory paths.

\subsection{Backend Pipeline}

The backend CI/CD pipeline handles the FastAPI application with the following stages:

\begin{enumerate}
    \item \textbf{Lint \& Test:} Sets up Python 3.11, installs dependencies from \texttt{requirements.txt}, runs \textbf{Ruff} for linting, executes \textbf{pytest} for unit tests (with mocked Supabase credentials), and verifies that the application module imports correctly.
    \item \textbf{Deploy to Modal:} On successful tests and a push to \texttt{main}, the pipeline deploys the backend to \textbf{Modal} using the Modal CLI. Deployment is gated by the \texttt{DEPLOY\_TARGET} repository variable.
\end{enumerate}

\textbf{Required Secrets:} \texttt{MODAL\_TOKEN\_ID}, \texttt{MODAL\_TOKEN\_SECRET}, and Supabase-related environment variables for the test stage.

\subsection{Frontend Pipeline}

The frontend CI/CD pipeline manages the React + Vite single-page application:

\begin{enumerate}
    \item \textbf{Lint \& Test:} Sets up Node.js 20, installs dependencies via \texttt{npm ci}, runs \textbf{ESLint} for linting, and executes \textbf{Vitest} for unit tests with dummy environment variables.
    \item \textbf{Build:} Produces a production bundle using \texttt{npm run build} with Supabase and API URL secrets injected at build time. The resulting \texttt{dist/} artifact is uploaded for subsequent jobs.
    \item \textbf{Deploy (Production):} On pushes to \texttt{main}, the pipeline uses the \textbf{Vercel CLI} to pull the production environment configuration, build, and deploy with the \texttt{--prod} flag.
    \item \textbf{Deploy (Preview):} On pull requests, a preview deployment is created on Vercel and the preview URL is automatically posted as a comment on the PR.
\end{enumerate}

\textbf{Required Secrets:} \texttt{VERCEL\_TOKEN}, \texttt{VERCEL\_ORG\_ID}, \texttt{VERCEL\_PROJECT\_ID}, \texttt{VITE\_SUPABASE\_URL}, \texttt{VITE\_SUPABASE\_ANON\_KEY}, and \texttt{VITE\_API\_URL}.
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{l X}
        ...
        \midrule
        \textbf{Nawriz Ahmed Turjo (2105032)} & Frontend , API Integration, FAISS Optimization , Hosting and CD \\
        ...
        \textbf{Shams Hossain Simanto (2105048)} & Backend, API Endpoint Creation, Recommendation Engine \\
        \textbf{Amit Saha (2105049)} & CI/CD Pipeline, API Unit Test Integration \\
        \textbf{Md. Abrar Jahin (2105055)} & Backend, API Endpoint Creation, Recommendation Engine, Model Quantization \\
        \bottomrule
    \end{tabularx}
\end{table}
